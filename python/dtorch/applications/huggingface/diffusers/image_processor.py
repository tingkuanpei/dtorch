...
def is_valid_image(image) -> bool:
    ...
    - A `PIL.Image.Image`.
    - A 2D or 3D `np.ndarray` or `dtorch.Tensor` (grayscale or color image).

    Args:
        image (`Union[PIL.Image.Image, np.ndarray, dtorch.Tensor]`):
            The image to validate. It can be a PIL image, a NumPy array, or a dtorch tensor.

    Returns:
        `bool`:
            `True` if the input is a valid image, `False` otherwise.
    """
    return isinstance(image, PIL.Image.Image) or isinstance(image, (np.ndarray, dtorch.Tensor)) and image.ndim in (2, 3)
...
def is_valid_image_imagelist(images):
    ...
    - A valid single image: `PIL.Image.Image`, 2D `np.ndarray` or `dtorch.Tensor` (grayscale image), 3D `np.ndarray` or
      `dtorch.Tensor`.
    - A list of valid images.

    Args:
        images (`Union[np.ndarray, dtorch.Tensor, PIL.Image.Image, List]`):
    ...
    """
    if isinstance(images, (np.ndarray, dtorch.Tensor)) and images.ndim == 4:
        return True
...
class VaeImageProcessor(ConfigMixin):
    ...
    @staticmethod
    def numpy_to_pt(images: np.ndarray) -> dtorch.Tensor:
        r"""
        ...
        images = dtorch.from_numpy(images.transpose(0, 3, 1, 2))
        return images

    @staticmethod
    def pt_to_numpy(images: dtorch.Tensor) -> np.ndarray:
        r"""
        Convert a PyTorch tensor to a NumPy image.

        Args:
            images (`dtorch.Tensor`):
    ...
    @staticmethod
    def normalize(images: Union[np.ndarray, dtorch.Tensor]) -> Union[np.ndarray, dtorch.Tensor]:
        r"""
        Normalize an image array to [-1,1].

        Args:
            images (`np.ndarray` or `dtorch.Tensor`):
                The image array to normalize.

        Returns:
            `np.ndarray` or `dtorch.Tensor`:
                The normalized image array.
        """
        return 2.0 * images - 1.0

    @staticmethod
    def denormalize(images: Union[np.ndarray, dtorch.Tensor]) -> Union[np.ndarray, dtorch.Tensor]:
        r"""
    ...
    def resize(
    ...
    ) -> Union[PIL.Image.Image, np.ndarray, dtorch.Tensor]:
        """
        Resize image.

        Args:
            image (`PIL.Image.Image`, `np.ndarray` or `dtorch.Tensor`):
        ...
        Returns:
            `PIL.Image.Image`, `np.ndarray` or `dtorch.Tensor`:
                The resized image.
        """
        ...
        elif isinstance(image, dtorch.Tensor):
            image = dtorch.nn.functional.interpolate(
                image,
                size=(height, width),
            )
    ...
    def _denormalize_conditionally(
        self, images: dtorch.Tensor, do_denormalize: Optional[List[bool]] = None
    ) -> dtorch.Tensor:
        r"""
    ...
    def preprocess(
        ...
        """
        supported_formats = (PIL.Image.Image, np.ndarray, dtorch.Tensor)

        # Expand the missing dimension for 3-dimensional pytorch tensor or numpy array that represents grayscale image
        if self.config.do_convert_grayscale and isinstance(image, (dtorch.Tensor, np.ndarray)) and image.ndim == 3:
            if isinstance(image, dtorch.Tensor):
        ...
        if isinstance(image, list) and isinstance(image[0], dtorch.Tensor) and image[0].ndim == 4:
            warnings.warn(
                "Passing `image` as a list of 4d dtorch.Tensor is deprecated."
                "Please concatenate the list along the batch dimension and pass it as a single 4d dtorch.Tensor",
                FutureWarning,
            )
            image = dtorch.cat(image, axis=0)
        ...
        elif isinstance(image[0], dtorch.Tensor):
            image = dtorch.cat(image, axis=0) if image[0].ndim == 4 else dtorch.stack(image, axis=0)
        ...
        if do_normalize and image.min() < 0:
            warnings.warn(
                "Passing `image` as dtorch tensor with value range in [-1,1] is deprecated. The expected value range for image tensor is [0,1] "
    ...
    def postprocess(
        ...
        """
        if not isinstance(image, dtorch.Tensor):
            raise ValueError(
    ...
    def pt_to_pil(self, image: dtorch.Tensor) -> PIL.Image.Image:
        return self.numpy_to_pil(self.pt_to_numpy(image))
...
class VaeImageProcessorLDM3D(VaeImageProcessor):
    ...
    @staticmethod
    def rgblike_to_depthmap(image: Union[np.ndarray, dtorch.Tensor]) -> Union[np.ndarray, dtorch.Tensor]:
        r"""
        Convert an RGB-like depth image to a depth map.

        Args:
            image (`Union[np.ndarray, dtorch.Tensor]`):
                The RGB-like depth image to convert.

        Returns:
            `Union[np.ndarray, dtorch.Tensor]`:
    ...
    def preprocess(
        self,
        rgb: Union[dtorch.Tensor, PIL.Image.Image, np.ndarray],
        depth: Union[dtorch.Tensor, PIL.Image.Image, np.ndarray],
        height: Optional[int] = None,
        width: Optional[int] = None,
        ...
        # Expand the missing dimension for 3-dimensional pytorch tensor or numpy array that represents grayscale image
        if self.config.do_convert_grayscale and isinstance(rgb, (dtorch.Tensor, np.ndarray)) and rgb.ndim == 3:
        ...
        if isinstance(rgb[0], PIL.Image.Image):
        ...
        elif isinstance(rgb[0], dtorch.Tensor):
            raise Exception("This is not yet supported")
            # rgb = dtorch.cat(rgb, axis=0) if rgb[0].ndim == 4 else dtorch.stack(rgb, axis=0)
            ...
            # depth = dtorch.cat(depth, axis=0) if depth[0].ndim == 4 else dtorch.stack(depth, axis=0)
...
class IPAdapterMaskProcessor(VaeImageProcessor):
    ...
    @staticmethod
    def downsample(mask: dtorch.Tensor, batch_size: int, num_queries: int, value_embed_dim: int):
        """
...
class PixArtImageProcessor(VaeImageProcessor):
    ...
    @staticmethod
    def resize_and_crop_tensor(samples: dtorch.Tensor, new_width: int, new_height: int) -> dtorch.Tensor:
        r"""
        Resizes and crops a tensor of images to the specified dimensions.

        Args:
            samples (`dtorch.Tensor`):
        ...
        Returns:
            `dtorch.Tensor`: A tensor containing the resized and cropped images.
        """
