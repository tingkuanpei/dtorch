...
from dtorch.distributed_spec import init_device_mesh, Shard, Replicate
...
from dtorch.applications.huggingface.transformers.models.clip.modeling_clip import (
    CLIPTextModel as DTorchCLIPTextModel,
)
...
def _test_clip(test_case, model_path, device, dtype):
    clip_model_path = os.path.join(model_path, "text_encoder")
    torch_input_ids = torch.randint(1000, 2000, (2, 64), device=device)

    def torch_imp():
        torch_model = TransformersCLIPTextModel.from_pretrained(clip_model_path).eval()
    ...
    def dtorch_imp(dp=1, tp=1):
        ...
        dtorch_model = DTorchCLIPTextModel.from_pretrained(
            clip_model_path,
            torch_dtype=dtype,
        ...
        if device_mesh.is_distributed:
            assert_tensor_allclose(test_case, torch_output, dtorch_output, rtol=5e-2, atol=5e-2)
        else:
...
def _test_t5(test_case, model_path, device, dtype):
    t5_model_path = os.path.join(model_path, "text_encoder_2")
    torch_input_ids = torch.randint(1000, 2000, (2, 256), device=device)
    torch_wo_weight_dtype = None

    def torch_imp():
        torch_model = TransformersT5EncoderModel.from_pretrained(t5_model_path, torch_dtype=dtype)
...
def _test_transformer_flux_1(test_case, model_path, small_fake_model, device, dtype):
    model_size = 5.7 if small_fake_model else 23
    memory_require = MemoryRequire(model_size, model_size)

    transformer_model_path = os.path.join(model_path, "transformer")
    torch_latents = torch.randn(1, 64, 64, device=device, dtype=dtype)
    torch_timestep = torch.tensor((1000.0,), device=device, dtype=torch.float32)
    torch_guidance = torch.tensor((3.5,), device=device, dtype=torch.float32)
    torch_pooled_prompt_embeds = torch.randn(1, 768, device=device, dtype=dtype)
    # Scale down the pooled projections to avoid overflow in the attention mechanism
    torch_pooled_prompt_embeds = torch_pooled_prompt_embeds / 10
    torch_prompt_embeds = torch.randn(1, 512, 4096, device=device, dtype=dtype)
    torch_text_ids = torch.zeros(512, 3, device=device, dtype=dtype)
    torch_latent_image_ids = torch.zeros(64, 3, device=device, dtype=dtype)

    def torch_imp():
        if not is_graph_satisfy(dtorch.default_graph, memory_require=memory_require):
            return

        torch_model = TransformersFluxTransformer2DModel.from_pretrained(
            transformer_model_path,
            torch_dtype=dtype,
        ).eval()
        torch_model = torch_model.to(device=device)
        torch_output = torch_model(
            hidden_states=torch_latents,
            timestep=torch_timestep / 1000,
            guidance=torch_guidance,
            pooled_projections=torch_pooled_prompt_embeds,
            encoder_hidden_states=torch_prompt_embeds,
            txt_ids=torch_text_ids,
            img_ids=torch_latent_image_ids,
        )
        torch_output = torch_output[0]
        del torch_model
        gc.collect()
    ...
    def dtorch_imp(dp=1, tp=1, ulysess_cp=1, ring_cp=1):
        ...
        dtorch_guidance = dtorch.Tensor(torch_guidance)
        dtorch_pooled_prompt_embeds = dtorch.Tensor(torch_pooled_prompt_embeds)
        dtorch_prompt_embeds = dtorch.Tensor(torch_prompt_embeds)
        dtorch_text_ids = dtorch.Tensor(torch_text_ids)
        dtorch_latent_image_ids = dtorch.Tensor(torch_latent_image_ids)

        dtorch_model = DTorchFluxTransformer2DModel.from_pretrained(
            transformer_model_path,
            torch_dtype=dtype,
            device_mesh=device_mesh,
        )
        ...
        dtorch_output = dtorch_model(
            hidden_states=dtorch_latents,
            timestep=dtorch_timestep / 1000,
            guidance=dtorch_guidance,
            pooled_projections=dtorch_pooled_prompt_embeds,
            encoder_hidden_states=dtorch_prompt_embeds,
            txt_ids=dtorch_text_ids,
            img_ids=dtorch_latent_image_ids,
        )
        dtorch_output = dtorch_output[0]
        if device_mesh.is_distributed:
            assert_tensor_allclose(test_case, torch_output, dtorch_output, rtol=2e-1, atol=2e-2, equal_nan=True)
        else:
            assert_tensor_equal(test_case, torch_output, dtorch_output)
        del dtorch_model
        dtorch.default_graph.empty_cache()
...
def _test_scheduler(test_case, model_path, device, dtype):
    ...
    torch_noise_pred = torch.randn(1, 4096, 64, device=device, dtype=dtype)
    torch_t = torch.tensor(1000.0, device=device, dtype=dtype)
    torch_latents = torch.randn(1, 4096, 64, device=device, dtype=dtype)
    torch_scheduler.set_timesteps(num_inference_steps, device=device, mu=mu)
    ...
    dtorch_latents = dtorch.Tensor(torch_latents)
    dtorch_scheduler.set_timesteps(num_inference_steps, device=device, mu=mu)
...
def _test_pipeline(test_case, model_path, small_fake_model, device, dtype):
    model_size = 6.5 if small_fake_model else 32
    same_device_plus = 5.7 if small_fake_model else 23
    ...
    def torch_imp():
        ...
        torch_pipe = DiffusersFluxPipeline.from_pretrained(model_path, torch_dtype=dtype)
        nonlocal torch_wo_weight_dtype
        torch_wo_weight_dtype = torch_pipe.text_encoder_2.encoder.block[0].layer[1].DenseReluDense.wo.weight.dtype
        torch_pipe = torch_pipe.to(device)
    ...
    def dtorch_imp(dp=1, tp=1, ulysess_cp=1, ring_cp=1):
        ...
        dtorch_pipe = DTorchFluxPipeline.from_pretrained(
            model_path,
            torch_dtype=dtype,
            device_mesh=device_mesh,
        )
        dtorch_wo_weight_dtype = dtorch_pipe.text_encoder_2.encoder.block[0].layer[1].DenseReluDense.wo.weight.dtype
        test_case.assertTrue(torch_wo_weight_dtype, dtorch_wo_weight_dtype)
    ...
    dtorch_imp()
    # dtorch_imp(tp=2)
    # dtorch_imp(ring_cp=2)
    # dtorch_imp(ulysess_cp=2)
    # dtorch_imp(ulysess_cp=2, ring_cp=2)


def _get_model_paths():
    """Get list of (model_path, small_fake_model) tuples for available Flux model paths."""
    paths = []
    small_fake_model_path = os.path.join(get_test_data_path(), "FLUX.1-dev-small-fake")
    ...
    model_path = os.path.join(get_test_data_path(), "FLUX.1-dev")
    ...
    def test_clip(self):
        ...
        for arg in gen_arg_list(arg_dict):
            for model_path, _ in _get_model_paths():
                _test_clip(self, model_path, *arg)
...
class TestT5(unittest.TestCase):
    ...
    def test_t5(self):
        ...
        arg_dict = OrderedDict()
        arg_dict["device"] = ["cuda"]
        arg_dict["dtype"] = [torch.bfloat16, torch.float16]
        for arg in gen_arg_list(arg_dict):
            for model_path, _ in _get_model_paths():
                _test_t5(self, model_path, *arg)

    def tearDown(self):
        dtorch.default_graph.empty_cache()


class TestTransformerFlux1(unittest.TestCase):
    @torch.inference_mode
    def test_transformer_flux_1(self):
        if not is_test_data_exists():
            return
        arg_dict = OrderedDict()
        arg_dict["device"] = ["cuda"]
        ...
        for arg in gen_arg_list(arg_dict):
            for model_path, is_small_fake_model in _get_model_paths():
                _test_transformer_flux_1(self, model_path, is_small_fake_model, *arg)
