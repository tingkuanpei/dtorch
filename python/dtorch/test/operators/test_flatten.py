...
def _test_flatten(test_case, shape, device):
    torch_in = torch.rand(*shape, dtype=torch.float32, device=device)
    dtorch_in = dtorch.Tensor(torch_in)

    torch_out = torch.flatten(torch_in, start_dim=1)
    dtorch_out = dtorch.flatten(dtorch_in, start_dim=1)
    assert_tensor_equal(test_case, torch_out, dtorch_out)

    torch_out = torch.flatten(torch_in)
    dtorch_out = dtorch.flatten(dtorch_in)
    assert_tensor_equal(test_case, torch_out, dtorch_out)
