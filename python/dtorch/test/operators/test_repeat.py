...
def _test_repeat(test_case, shape, device):
    torch_in = torch.rand(*shape, dtype=torch.float32, device=device)
    dtorch_in = dtorch.Tensor(torch_in)

    torch_out = torch_in.repeat(repeats=(2, 3, 1))
    dtorch_out = dtorch_in.repeat(repeats=(2, 3, 1))
    assert_tensor_equal(test_case, torch_out, dtorch_out)

    torch_out = torch_in.repeat(2, 3, 0)
    dtorch_out = dtorch_in.repeat(2, 3, 0)
    assert_tensor_equal(test_case, torch_out, dtorch_out)
